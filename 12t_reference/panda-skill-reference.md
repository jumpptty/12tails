...
- **Dispatch (`doSkill`, `Panda.cs:6851-6936`):** `tDir` = normalized flat direction to the target (`:5648`). With **Wind Walker (#423)**: distance > **6 m** (`sqrMagnitude > 36`) → if > **16 m** (`> 256`) shows `"Too far to use WindWalker"`, `returnMPSP(text)` and cancels; otherwise `tDir = getSpawnPos(target − 3·dir) − pos`, i.e. a spot **3 m in front of the target** (`:6861-6914`). The coroutine warps there in state 2 only when `tDir.sqrMagnitude > 36` (`:34192-34200`), i.e. when that spot is > 6 m away, so the warp effectively needs the target **> ~9 m** away (see Wind Walker below).
...
### Death Blow (`panda_deathBlow`, skill IDs #323/#324)

- **Cost / requirements (`decode_skilldata.py`):** MP **15, 20**, SP **18, 24** (blue threshold, not consumed), Lv 19/25, Bn 6/8, mode target, target enemy.
- **Tooltips:** EN `"…has a 10% / 15% chance to instantly kill the target. Success rate also depends on target's current hp."`; TH `"กระโดดพุ่งเข้าสะกัดจุดตาย ของเป้าหมาย โอกาสสำเร็จ ตามค่าต่าง hp (10%/15%success)"` (`PandaSkill_eng.cs:660-682`, `PandaSkill_thai.cs:682-704`). Wind Walker EN: `"…Also double DeathBlow's chance of inflicting death."` (`PandaSkill_eng.cs:1016`).
- **Dispatch / movement / hit box:** identical to Stasis Blow: same Wind Walker warp block (`Panda.cs:6768-6830`, 6 m / 16 m dispatch thresholds, refund beyond 16 m; actual warp only for targets ~9–16 m, see Wind Walker), same coroutine timeline (`$RPC_deathBlow$25547`, `:34851-35610`: speed 3 → 10 with target collision ignored → 4, yields 0.2/0.1/0.2/0.1/0.2 s, **4.7 m**), same box `FindRecTarget(pos − 2·forward, forward, 1+sf, 1+sf, (4+sf)·rangeMod, 2·rangeMod)` (`:35128`). `addTimeOut("deathBlow", agiAdjust(30))` (`:35421`).
- **Hit:** `hit(322 + sLv, target, (int)(0.4·ATK + talAdjust(5 + 10·sLv)), 1, 0, zero)` (`:35151`), KO **1**; on a landed hit `sp += 1` and `ShadowFist(target)`.
- **Kill roll (`:35180-35211`):** only if the target's `actionState` is `standby`, `run` or `emotion`: `if (lckAdjust(20·sLv) > Random.Range(0, target.hp)) target.RPC_AddStatus("death", 1, 1, 0, …)`. `Random.Range(int,int)` excludes `hp`, so with `L = lckAdjust(20·sLv)` the success chance is exactly **`min(L, hp) / hp`**: any target at **≤ L HP always dies**. `L` = 20 / 40 at LCK ≤ 1, e.g. 33 / 57 at LCK 100, 42 / 66 at LCK 200. There is **no fixed 10% / 15% term**.
- **Wind Walker does not change the chance:** `hasSkill(423)` appears in `Panda.cs` only in the two warp dispatches (`:6768`, `:6861`); the kill roll has no Wind Walker term, and `windWalker` is referenced nowhere outside `Panda.cs` / `PandaSkill*.cs`. The tooltip's "double chance" is not in the client (possibly server-side; not live-tested).
- **`death` status:** code **307** (`StatusData.cs:844`), **Debuff + System** (`isDebuffStatus` `:7340`, `isSystemStatus` `:4762`). Apply sets **`hp = 0`** (`CharacterControl.cs:35416-35420`). Targets with `recieveStatus == false` ignore every status (`RPC_AddStatus` entry); which monsters have it is serialized data, not in this code.
- **Card modelling:** `targetHpInput:true` adds an HP pill (default 1000); `lckProc.calc` = `floor(1000·min(L, HP)/HP)/10` %, `baseText` shows the always-kill threshold `L`, `simulate:false`.

### Wind Walker (`panda_windWalker`, skill ID #423)

- **Passive, Lv 70 / Bn 3, 0 MP / 0 SP** (`decode_skilldata.py`; `PandaSkill.cs:1252`, tree id `:3232`).
- **Tooltips:** EN `"Gives StasisBlow and DeathBlow the ability to warp Panda to target's location. Also double DeathBlow's chance of inflicting death."` (`PandaSkill_eng.cs:1012-1016`); TH `"ทำให้แพนด้าสามารถหายตัวไปสกัดจุดเป้าหมายที่อยู่ระยะไกลได้ เพิ่มโอกาส deathBlow เป็นสองเท่า"` (`PandaSkill_thai.cs:1034`).
- **Only hooks:** `hasSkill(423)` in the Death Blow (`Panda.cs:6768-6830`) and Stasis Blow (`:6861-6914`) branches of `doSkill`. No other reference in `Panda.cs` or any other file (`windWalker` appears only in `Panda.cs`/`PandaSkill*.cs`; other classes' `hasSkill(423)` are their own skills).
- **Logic:** `d` = flat distance to the target. `d > 16` (`sqrMagnitude > 256`) → `"Too far to use WindWalker"`, `returnMPSP`, cast cancelled. `d > 6` → `tDir = getSpawnPos(target − 3·dir) − pos`. `Math.getSpawnPos` (`Math.cs`) raycasts down 4 m from 2 m above the point and returns the ground hit, else tries up to 10 random points within a 3 m sphere, else `Vector3.zero` (then `tDir` stays the unit direction). The coroutines warp (`position = mPos + tDir`) only when `tDir.sqrMagnitude > 36` (`:34192`, `:35009`). On flat ground `|tDir| ≈ d − 3`, so the **warp happens only for `d` ≈ 9–16 m**, landing ~3 m in front of the target; for 6–9 m nothing warps (only the facing uses `tDir`).
- **No kill-chance change:** the Death Blow roll (`:35180-35211`) has no Wind Walker term. The tooltip's "double chance" is not in the client.
