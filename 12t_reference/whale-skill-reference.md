...
| `whale_javelin` | Javelin | 2 | 10 MP, [8, 12] SP (red) | 30s | 0s | — | `0.5×ATK + talAdjust(10×sLv + 10×depLv)` | 5 | Pierces enemies in a straight line dealing 0.5×ATK + talAdjust(10×sLv + 10×depLv), KO=5 (corrected 2026-09-24, `Whale_javelin.cs:314`), applying puncture. Knight of the Deep reduces CD by -10s. Tide Cutter adds +10 TAL. |
...
| `whale_flyingShield`| Flying Shield | 2 | 0 MP, [16, 20] SP (red) | 45s | 0s | — | `Floor(0.5×sLv×DEF) + talAdjust(10×sLv)` | 5×sLv | Throws shield dealing Floor(0.5×sLv×DEF) + talAdjust(10×sLv), KO=5×sLv (corrected 2026-09-24, `Whale_flyingShield.cs:210`) to all targets in its path. Knight of the Deep reduces CD by -15s. |
...
- **Wall Puncture proc (verified 2026-09-24):** `getWallPuncture()` (`Whale.cs:9196`) = `lckAdjust(10·wallPunctureLv) > Random.Range(0,100)`, rolled per target per hit at 8 sites: Combo `nAttack1-3` (`:16238`, `:17128`, `:17492`, `:18310`), Sweep (`:21763`, `:22012`), Peninsula Impale (`:26844`), Javelin (`Whale_javelin.cs:276`). A proc **replaces** the hit with `RPC_AddEffectDamage(251, <raw>, KO 0)` + `puncture` (level = rank, 1 s; strips iron/diamond/perfect/bubble/ice shields and salvation, `CharacterControl.cs:35859+`). Javelin's purple raw is `talAdjust(10·lv)` **without** Tide Cutter's +10 (`Whale_javelin.cs:299` vs `:314`). Bible: `effectProc` replace on Sweep/Javelin/Impale.
...
- **Shield Reflect (#411, Lv 60 / Bn 1, passive):** three effects, all only while the Whale holds the charge: (1) `nKo = 0` for any direct hit while `actionState == "attack"` and `myCommand == "cAttack1"` (`CharacterControl.cs:4059-4075`, in `RPC_AddDamage`); (2) extra `nDamage −= 0.25·Lv`; (3) reflect `min(nDamage_before, floor(10 + 10·sLv + 0.25·Lv))` as Effect Damage (both in the `shield` branch above). Tooltips: EN `"…reflect 40 damage back to attacker."` (the rank-3 charge value, ignoring the `0.25·Lv` part), TH `"…สะท้อนดาเมจที่ถูกหักกลับใส่ศัตรูและเพิ่มอัตราการหักดาเมจเท่ากับ 25% ของเลเวลวาฬ"`; neither mentions the KO immunity.
