...
- **`warFactory`'s real cast site is `RPC_cartBomb`, not a dedicated `RPC_warFactory` — the "five secret
  ...
  ("War Factory - X") with real damage-formula citations — see `mole-skill-reference.md`'s own
...
|---|---|---|---|---|---|---|---|
| reload | 2 | [15, 20] MP | none | no dmg — cooldown-reset/SP-restore utility | — | — | — |
| mine | 4 | [3, 5, 7, 9] MP | none | `talAdjust(10×sLv+10)` | AoE, single hit per target | **mineLover** ×1.25 mult (hasSkill 402) | 1 |
| mortarShot | 2 | [12, 16] SP (blue) | none (only shell *count* scales) | `0.5×ATK + talAdjust(15)` | per shell, `2×sLv+1` shells; falls off with distance; ×2 vs Structure race | none modeled | `2×sLv+1` |
| bunker | 2 | [5, 10] SP (red) | none | no dmg — self-status, −50%/−75% incoming dmg + KO immunity | — | — | — |
| tnt | 4 | [10, 15, 20, 25] SP (red) | none (uses highest TNT rank *owned*, not necessarily cast rank) | `talAdjust(20+10×sLv)` | falls off with blast distance | **superTNT** `(0.1×sLv+0.1)×LV` additive (hasSkill 422) | 1 |
| stunMine | 2 | 5 MP, [10, 15] SP (red) | none | no dmg — pure KO (`10×(sLv+[grenadeCluster5:+1])`, ×1.25 if mineLover) | — | — | — |
| stunGrenade | 2 | [5, 10] MP, [10, 15] SP (red) | none | no dmg — pure KO (`10×(sLv+[grenadeCluster5:+1])`); +30 flat REAL damage vs enemy-layer targets if caster has Smart Shell (not modeled) | — | — | — |
| flameTurret | 3 | [24, 28, 32] SP (red) | none | `talAdjust(5×sLv)` per tick, **penetrating — bypasses defAdjust entirely** (direct `RPC_AddDamage`, same mechanism as `napalm`) | organic-race-only; ticks = `chaAdjust(5)` duration ÷ 0.25s | none found | `duration/0.25` (`hitCountDuration:0.25`) |
| fireBarrage | 2 | [10, 20] MP, [20, 35] SP (blue) | none | `talAdjust(5+10×sLv)` | per volley-hit | none found | `4×(sLv≤1?1:3)` |
| bombardment | 2 | [20, 35] MP, [35, 50] SP (blue) | none | `talAdjust(10+15×sLv)` | per strike-hit; falls off with distance | none found | `5×(sLv≤1?1:3)` |
| timeNuke | 2 | [45, 60] MP, [45, 60] SP (red) | none | `talAdjust(mLv×100+50)`, mLv=sLv | up to 4 ring pulses, falls off with ring distance | **smartShell1**: halves same-layer / +30 flat enemy-layer (not modeled) | up to 4 |
| detonate | 1 | 24 MP, 24 SP (red) | none | shares timeNuke's base, scaled `clamp(elapsed×0.0166,0.1,0.99)` | manual early-trigger; damage grows 10%→99% with arm time — no Damage Formula chip at all in the tool (2026-08-19, removed entirely rather than shown as opaque text; see Time Nuke's own row for the shared base) | (inherits timeNuke's deps) | up to 4 |
| autoGyroGun | 4 | [10, 15, 20, 25] MP, 10 SP (red) | **castTime**: `magAdjust(1+sLv)` = [2,3,4,5] (corrected from a flat 5) | `10×sLv` (turret's own flat ATK stat, NOT Mole's TAL), KO=1 (flat, not rank-scaled — `AutoGyroGun.cs:1243`) | continuous turret AI, not a single cast; base is REPLACED (not added to) by hiddenTurret5, see below | **hiddenTurret5** (replace, see below); **synchroMole2** (additive `+TAL`, see below) | continuous |
| barrelBot | 4 | [25, 35, 45, 55] MP, 10 SP (red) | **castTime**: `magAdjust(3+sLv)` = [4,5,6,7] (corrected from a flat 7) | no dmg on the base summon entry itself — its 8 individual moves are modeled as their own `SKILLS` entries (see "Barrel Bot's own moveset" below) | Barrel Bot has its own independent `CharacterControl`/stat block (see below), not a proxy for Mole's | doubleBot5 (+0.5×LV to all stats) and synchroMole2 (+TAL to atk) both **confirmed** (see below) | continuous |
| megaPunch | 2 | [15, 18] SP (blue) | none | `0.5×ATK + talAdjust(10×sLv)`, KO=5 | tooltip omits the ATK term | **megaDrill** ×1.5 mult on dmg (and KO, not shown) (hasSkill 433) | 1 |
| megaHammer | 2 | [18, 20] SP (blue) | none | `0.5×ATK + talAdjust(12×sLv)`, KO=`10×sLv` | tooltip omits the ATK term | **megaDrill** ×1.5 mult on dmg (and KO, not shown) | 1 |
| chopper | 3 | [20, 24, 28] SP (blue) | none | `0.3×ATK + talAdjust(5×sLv)` per tick | forward-flight collision ticks | **barrelCannon5** adds a separate flat `talAdjust(20)` AoE bombing pass, once per tick (not modeled — different formula shape) | 10 |
| missile | 4 | [12, 16, 20, 24] MP | none | `talAdjust(30)` flat, per missile | homing, random enemy within 32m | none found | `sLv` (1–4) |
| synchroMole | 2 | [15, 25] MP, [25, 35] SP (red) | none | no dmg — self/ally buff, +50%/100% of Mole's TAL to summons' ATK/DEF | — | — | — |
| kingKaiser | 1 | 80 MP, 80 SP (red) | none | no dmg by itself — 240s mech-transform buff; unlocks separate kaiserCannon1/kaiserBeam1 mech attacks (out of this 27-skill roster, not itemized) | — | — | — |
| advanceRepair | 1 | 10 SP (red) | none | no dmg — heals 150hp/sec, drains 1sp/sec | — | — | — |
| napalm | 1 | 40 SP (red) | none | 1st tick `talAdjust(30)`, ticks 2–6 `talAdjust(5)` each — modeled in the tool via `dmgGroups` (2026-08-19), the first skill whose own hits roll genuinely different base formulas rather than one formula×hitCount | organic-race-only; direct `RPC_AddDamage`, bypasses both `dmgAdjust` and `defAdjust` ("penetrating"); **each of the 5 fired projectiles independently starts its OWN full 6-pulse sequence on impact** (`Mole_napalm.cs:179`'s `OnTriggerEnter` -> `RPC_napalm_hit`, confirmed the ONLY trigger source — `Mole.cs`'s 2 other call sites, `:2831`/`:5671`, are both `if (this.mChar.isMine) break`-guarded Photon replication receivers, not independent triggers) — so a target all 5 projectiles connect with (reliable in practice: cast point-blank, large pulse hitbox) takes 5×6 = **30 total pulses**, not 6 | none found | 30 (`hitCount:()=>30`, `dmgGroups:[{hitCount:5},{hitCount:25}]`) |
| grenadeCluster | 1 | 10 MP, 40 SP (red) | none | `ATK + talAdjust(30)`, KO=10, per grenade | self-centered ring burst; falls off with distance | none found | 8 |
| flameCarnival | 1 | Free / 0 | none | `talAdjust(10)` per tick, organic-race-only | radius `clamp(2+i,3,5)`; Duration column is the unarmed trap's arming window, NOT the burn time | **smartShell1** halves same-layer dmg (not modeled) | 20 (hardcoded, not CHA-scaled) |
| megaDrill | 1 | 36 SP (blue) | none | `ATK + talAdjust(45)`, KO=2 | forward drill dash | none found | 1 |
| barrelCannon ("Barrel Cannon" — briefly renamed "Cannon Form" mid-session 2026-08-20, reverted back the same day since that's the skill's real skill-tree name) | 1 | 50 SP (blue) | none | `1.5×ATK`, Barrel Bot's OWN ATK/attacker-LCK (`ownStatsDmgOnly:true`) — **resolved** (was "unresolved" here; the shell-impact formula was found in the same session, see "Barrel Bot's own moveset" below and the Cannon Form merge note) | the ONLY Barrel Bot move Mole explicitly commands rather than the AI picking it — merged with the former `SKILLS` entry `mole_barrelBot_cannonForm` (2026-08-20); Cooldown is Mole's OWN 120s cast-side cooldown (`Mole.cs:40199`, Mole's own AGI/LCK, revisedArt-eligible) — NOT Barrel Bot's own 90s internal re-trigger timeout (`BarrelBot.cs:5639`, still real but not the one shown, see the merge note below) | (also enables chopper's bombing pass) | 10 |
...
| warCapital | 1 | 100 MP, 50 SP (red) | **castTime**: `magAdjust(24)` flat, confirmed correct, unchanged | `talAdjust(50)` per missile, KO=3 | 8-missile volley | none found | 8 |
