...
* **Inline Skill Description Container (`.sk-hero-desc`):**
  ...
  * **`desc` Inline Markdown (added 2026-09-18):** Three independent bracket/asterisk conventions, all parsed by the same handful of `.replace()` calls scattered across `renderHero()`/the support-level popup/the changelog server-diff popup — when adding a 4th call site for any of these, wire all three, not just `**`:
    ...
    - **`[statusName]` → purple hoverable status keyword** (`<span class="sk-status" data-classification="...">`, matching `sk-status`'s existing look). Optional trailing rank digits are literal typed text, not computed (`[frost]`, `[dissolute4]`) — see the next bullet for the classification source and how this interacts with a card's own `status:` field.
  * **Status Keyword Rendering (`STATUS_CLASS_MAP`, `renderStatusKeywords()`, added 2026-09-18):**
    ...
    - A status name not yet in the map renders as **plain literal bracket text** (safe no-op fallback, not an error) — an easy visual signal that it still needs research before it'll show purple.
    ...
    - `renderStatusKeywords()` skips any `[name]` that's already hand-wrapped in its own `<span class="sk-status">` (legacy per-card authoring predating this convention, e.g. `bat_dissolute`'s inline `kw2` template) — otherwise it double-wraps. New cards should use the bare `[name]` shorthand, not hand-rolled `<span>` HTML.
