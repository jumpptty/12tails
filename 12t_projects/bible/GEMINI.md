...
A hub tool (`mountChaAgiOptimizer`, tile "ตัวคำนวณ CHA / AGI") that shows, per skill, the cheapest CHA + AGI for **no downtime** (cooldown ≤ duration, so the buff can be recast before it ends) No stat inputs and no explanatory text lines (user decision): each card is two numbers, the target CHA and AGI, each with the base value its stat scales (CHA box: base duration, AGI box: base cooldown). Only the final target is shown for now (user decision 2026-09-25): the ~61 rule line and the LCK 0 / Double Art / Perseverance footnote were removed.
