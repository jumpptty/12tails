...
A hub tool (`mountChaAgiOptimizer`, tile "ตัวคำนวณ CHA / AGI") that shows, per skill, the cheapest CHA + AGI for **no downtime** (cooldown ≤ duration, so the buff can be recast before it ends) and one **AGI threshold**. No stat inputs and no explanatory text lines (user decision: each card is just three numbers, CHA / AGI / เกณฑ์ AGI +gap).
...
* **Math** (top-level, validator-callable): `caoDuration` = `floor(dur × (1 + 0.015 × clamp(CHA,1,512)))`, then Perseverance `floor((1.1+0.2n)×d)`; `caoCooldown` = `cd × 128/(AGI+128)`, Revised Art `ceil(0.88×c)`; LCK fixed at 0 (worst case). `caoOptimal` brute-forces CHA 0–512 with `caoMinAgi` and returns `{cha, agi, dur, cd, gap}`.
* **AGI threshold = `gap` = CHA − AGI at the optimum.** Rule on the card: put points into CHA; once CHA leads AGI by `gap`, put the next point into AGI (keep the lead). From 0/0 that path visits one pair per total, so its first no-downtime pair is the optimum itself. The smooth-math version of the same idea is `CHA < AGI + 61.33` (from `(CHA+66.67)(AGI+128) ≥ K²`); the per-skill `gap` differs from 61 because of the game's floor/ceil rounding.
...
* **Validator:** `[CAO ERROR]`: 36 cooldown/duration/Perseverance/Revised Art cases against an independent brute force, the threshold path landing on the optimum, known totals (Dark Edge 522 / 354, Lunar Eclipse 632 / 439), and the real mount (4 cards, toggle re-render, Ctrl+Z).
