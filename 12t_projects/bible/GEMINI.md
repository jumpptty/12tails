...
* **`KNOWN_RANGE_SIM_MISMATCH`** in the validator lists skills that still disagree, each with a reason (currently only Sheep Over Heal). It only suppresses; remove an entry as soon as its skill is consistent. Do not widen a range to make a skill pass.
