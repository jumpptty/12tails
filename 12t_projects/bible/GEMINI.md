...
* **Inline Skill Description Container (`.sk-hero-desc`):**
  ...
  * **Status Keyword Rendering (`STATUS_CLASS_MAP`, `STATUS_DESC_MAP`, `renderStatusKeywords()`, `renderStatusTip()`, added 2026-09-18):**
    ...
    - **Status mentions inside `STATUS_DESC_MAP`** use the same `[name]` renderer, so a popup can link to another status (for example, `[darkEdge]` mentions `[holyWolf5]`). Cycles are cut at the repeated name: that final mention stays purple without creating another popup. Popup visibility selectors target only direct child tips so opening one status does not open every nested tip.
...
* **Cat Power Number Series Toggle (`CAT_POWER_DEP`, added 2026-09-25):**
  ...
  * **Related links on TTO (2026-09-26):** Combo (non-Class-A) also drops Power Seven / Super Seven from its description and related-skills list on TTO (`servers.tto.compatSkills` / `desc`), and Power Seven drops the Combo link there. The TTO button only exists for a card with a `servers.tto` entry (`getAvailableServers`), so every non-Class-A Cat damage card carries `servers:{tto:{changeNote:"• [NERF] Power 1 2 3 7 and Super Seven มีผลกับแค่สาย Class A"}}` (add it to every new Cat Tree B / Class C damage card). Validator §3q (`[CAT POWER TTO ERROR]`) checks every Cat damage card: no Power toggle on TTO unless Tree A, and the displayed range ignores a stale Power rank.
* **Open Wound toggle and description line (`CAT_OPENWOUND_PROC`, 2026-09-26):** one on/off icon (`openWound`, default off). Off = no bonus; on assumes the target already has Disarm Lv 2 and Bleed Lv 2 (`30 x (2 + 2)` = 120 purple per landed hit); on Disarm/Bleed the same toggle also adds the +3 s to the contested duration. Every card that carries it ends its `desc` with `**Open Wound:** ดาเมจม่วงเพิ่มเติมทุกฮิต`, or, when `effectProc.hits` limits it, `**Open Wound:** ดาเมจม่วงเพิ่มเติม__เฉพาะฮิตที่ x,y__` (red). Combo uses its own wording, "ทำ ^^ดาเมจม่วง^^ เพิ่มเติมทุกฮิต __แต่หากฟันหลังด้วย **Hidden Blade** ฮิตที่ 2 จะไม่ติดดาเมจม่วง__" (`catComboOpenWoundLine`).
...
> **5-column main stats row (`stats5: true`, added 2026-09-26 for Arctic Wind):** the main `.sk-hero-stats` grid is normally a fixed 4 columns (Cooldown/Cast Time/Duration/LCK-proc, in that column order), which collides when a card has both a `secondaryDuration` (column 4) and a `lckProc` (also column 4 by default) at once — Arctic Wind's Cooldown, Cast Time, Duration Ice, Duration Frost and Frost proc chance are 5 genuinely distinct facts that all belong in one row. A card sets `stats5: true` to widen its own `.sk-hero-stats` to 5 equal columns and move the primary `lckProc` chip into its own column 5 (`cd`/`cast`/`dur`/`dur2` keep columns 1-4 unchanged, no `chipCols` override needed). At the ≤680px extreme-narrow breakpoint it collapses to 3 columns, wrapping 3+2 (`cd`/`cast`/`dur` then `dur2`/`lck`). This is a distinct mechanism from `lckOdds`'s `.sk-odds-row` (a dynamic, auto-flowed *second* row of many same-shaped outcome chips below the main stats) — `stats5` instead widens the *existing* main row for a fixed, small set of already-named chip types. Only apply it to a card that genuinely needs all 5 slots simultaneously; a normal 2-4 stat card should never set it.
...
* **`effectProc.hits: [n, ...]` or `(rank) => [n, ...]` (added 2026-09-26, Cat Open Wound):** 1-based hit numbers (across all `dmgGroups`, in order) that carry the `bonus`; absent = every hit. The simulator skips the bonus on other hits, and the `dmgGroups` final range subtracts one bonus per skipped hit from the max. Finishing Blow `[3]` (only its third fixed hit calls `OpenWound`, `Cat.cs:38923`); Combo `()=>Hidden Blade on ? [1,3,4,5,6] : [1..6]` (stage 2's Hidden Blade hit lacks the call, `Cat.cs:17390-17507`). Checked in validator §3m.
...
* **To wire when each card gets its `/sd` pass** (found by a source survey, all confirmed white + purple in one cast): Whale Combo 1–3 (Wall Puncture, replace), ~~Wolf Combo 1–4~~ (done 2026-09-24 with `effectDamageDep`, see below), Cat Open Wound #443 (bonus `30×(disarm+bleed)`, no gate on the damage, only the flash/message has a 0.1 s throttle; wired 2026-09-26 via `CAT_OPENWOUND_PROC` to Combo, Flying Dagger, Forward Lunge, Reverse Thrust, Heart Ripper, Finishing Blow (hit 3 only), Disarm, Bleed, Moon Blade, Moon Storm and Delta Strike; see `effectProc.hits` below), Mole Smart Shell (bonus 30: Time Nuke, Stun Grenade, Mine, Stun Mine), the remaining Panda Shadow Fist users (Combo 1–5). Needing their own design: Bison Colossal Weapon (splash onto *other* targets) and Bat Merciless Drain (uncertain decompiled condition).
...
### CHA / AGI Optimizer tool (`cha-agi-optimizer`, added 2026-09-25, simplified the same day)

A hub tool (`mountChaAgiOptimizer`, tile "อยากสกิลวน ใช้แต้มน้อยสุดเท่าไหร่", short title "สกิลวน"; renamed from "ตัวคำนวณ CHA / AGI" 2026-09-25) that shows, per skill, the cheapest CHA + AGI for **no downtime** (cooldown ≤ duration, so the buff can be recast before it ends) No stat inputs and no explanatory text lines (user decision): each card is two numbers, the target CHA and AGI, each with the base value its stat scales (CHA box: base duration, AGI box: base cooldown). Only the final target is shown for now (user decision 2026-09-25): the ~61 rule line and the LCK 0 / Double Art / Perseverance footnote were removed.

* **Skills:** fixed list `CAO_SKILL_IDS` (user-chosen: Wolf Dark Edge, Wolf Lunar Eclipse, Rabbit Rapid Trance, Chameleon Immunity). Cooldown/duration/rank/icon are read from each skill's own `SKILLS` card, so card fixes flow through; add a skill only after checking its cast site uses a plain `agiAdjust(cd)` cooldown and a plain `chaAdjust(dur)` duration. Wolf Perseverance applies to a card whose `dep` is `WOLF_PERSEVERANCE_DEP`. Controls: Revised Art toggle on the title row; Perseverance 0/1/2 as a small icon toggle inside each Wolf card head (one shared state); per-card rank buttons.
* **Custom skill card (2026-09-25):** a 5th card, "สกิลกำหนดเอง", after the skill cards. The user types the base duration (under CHA, whole seconds only, since `chaAdjust` takes an `int`) and the base cooldown (under AGI, decimals allowed, `agiAdjust` takes a float); it has its own Perseverance toggle (`state.custom.persev`, independent of the Wolf cards) and follows Revised Art. `caoOptimal` runs on every keystroke (<1 ms); only the two result numbers are rewritten, so the inputs keep focus. Empty or 0 shows "–". Input edits are undoable (snapshot on focus, commit on change); Ctrl+Z inside a text field is left to the field.
* **Per-card override (2026-09-25):** the CHA and AGI numbers on every card (custom included) are editable in place (`.cao-big` inputs styled as the big numbers). Typing CHA solves the lowest AGI that still cycles (`caoMinAgi`); typing AGI solves the lowest CHA (`caoMinCha`, 0–512, "–" when even CHA 512 is not enough). `state.overrides[cardId | "custom"] = {stat, value}`, resolved by `caoPair()`; it survives rank / Revised Art / Perseverance / custom-base changes, which re-solve the other stat. "↺ ค่าต่ำสุด" (shown only when overridden) deletes the override. Typing updates the partner field in place so focus stays; undo snapshot on focus, commit on blur; Enter blurs.
* **Math** (top-level, validator-callable): `caoDuration` = `floor(dur × (1 + 0.015 × clamp(CHA,1,512)))`, then Perseverance `floor((1.1+0.2n)×d)`; `caoCooldown` = `cd × 128/(AGI+128)`, Revised Art `ceil(0.88×c)`; LCK fixed at 0 (worst case). `caoOptimal` brute-forces CHA 0–512 with `caoMinAgi` and returns `{cha, agi, dur, cd}`.
* **Rule for builds below the target (analysis kept, line not shown for now): "ลง CHA ให้นำ AGI ~61 แต้ม จน CHA ถึงเป้า แล้วที่เหลือลง AGI".** The measure that matters is **uptime = duration ÷ cooldown** (share of time the buff is on), not seconds of downtime per cycle. `uptime ∝ (CHA + 66.67) × (AGI + 128)` times a constant (Revised Art and Perseverance only change the constant), so +1 CHA beats +1 AGI while `CHA + 66.67 < AGI + 128`, i.e. keep CHA ~61.33 ahead, the same for every skill and toggle. Exact-rounding sweep, max-rank skills, every budget up to the target: leads 55–80 all within ~0.05 pp average of each other (best 72–73 by 0.02 pp, from rounding not Revised Art); at 61 the rule averages −0.76 pp (RA on) / −0.61 pp (RA off) vs the best split, worst −4.3 pp. Capping each stat at its target makes it land exactly on no downtime.
* **"AGI first" is wrong (tried and reverted 2026-09-25).** In seconds, +1 AGI removes more cooldown than +1 CHA adds duration (Rapid Trance at 0/0: −0.82s vs +0.18s), but a cooldown second is worth only *uptime ×* a duration second: +1s duration → `(d+1)/c`, −1s cooldown → `d/(c−1)`, and `(d+1)(c−1) − dc = (c − d) − 1`, so a duration second wins whenever downtime is over 1s. Rapid Trance RA on, all 355 budgets: CHA-lead wins 328, AGI-first 19 (by ≤1.3 pp, rounding ties at points 1–5 and 293–336), 8 ties; CHA-lead leads by up to 14.8 pp mid-way (AGI-first is up to 17–20 pp behind the best split on max-rank skills). Seconds of downtime only picks the right target, not the order.
* **Not a fixed AGI number:** in uptime terms the switch point `AGI + 129 < CHA + 66.7` moves with CHA, so it is stated as the ~61 lead. An exact partial-build calculator (current CHA/AGI + points to spend, brute force) was considered and skipped: +3 inputs for ~0.76 pp average (user chose simplicity).
* **Undo:** Ctrl+Z restores the previous toggle/rank state (50 deep), only while the tool is visible (`root.hidden` guard). The Skill Details enemy-preset Ctrl+Z got the same guard.
* **Validator:** `[CAO ERROR]`: 36 cooldown/duration/Perseverance/Revised Art cases against an independent brute force, override solving (the solved stat cycles and one point less does not; "–" exactly when CHA 512 is not enough), known totals (Dark Edge 522 / 354, Lunar Eclipse 632 / 439), and the real mount (4 cards, toggle re-render, Ctrl+Z).
