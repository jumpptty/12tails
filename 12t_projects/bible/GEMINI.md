...
* **Inline Skill Description Container (`.sk-hero-desc`):**
  ...
  * **Status Keyword Rendering (`STATUS_CLASS_MAP`, `STATUS_DESC_MAP`, `renderStatusKeywords()`, `renderStatusTip()`, added 2026-09-18):**
    ...
    - **No repetition between `STATUS_DESC_MAP` and the card `desc` (user rule 2026-09-24):** the status popup and the skill's own `desc` are read together, so a fact goes in exactly one of them. When a card's `desc` already says what its status does (e.g. Ashura's SP gain), the `STATUS_DESC_MAP` entry holds only what the `desc` leaves out (Ashura: `"ติดสถานะ provoke จะไม่ฟื้น SP"`), or there is no entry at all. Check both texts before adding either.
