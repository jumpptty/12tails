...
* **Inline Skill Description Container (`.sk-hero-desc`):**
  ...
  * **Mechanic-Glossary Links (`^^term^^`, `MECHANIC_DESC_MAP`, added 2026-09-22):** A 5th independent bracket/asterisk convention, for general game-engine mechanics that aren't tied to a status effect (e.g. how `moveSpeed`/`moveMod` combine, the damage pipeline) and are too multi-step for a small `[statusName]` hover tip.
    - **`^^term^^` → teal clickable link** (`<button class="sk-mech-link">`, `--mech-kw` token, `#0d9488`/`#2dd4bf`) that opens a full centered modal (`#mechanicModal`, same weight-class as the Buff/Debuff popup — not a small anchored tooltip). Unlike `**bold**`'s skill-link resolution, the literal text between `^^ ^^` is always both the lookup key (case-insensitive) and the displayed text, matching `[statusName]`'s convention. An unmapped key renders as plain literal text (safe no-op fallback).
    ...
    - **`MECHANIC_DESC_MAP`** (declared right before `getStatusDesc()`) holds `{title, steps:[{label, body}]}` per key — a structured step-list, not a flowing paragraph, since these topics are inherently sequential (the damage pipeline especially). `body` is rendered through `formatDescTokens()`, so a panel's own step text can cross-link to another mechanic panel via `^^otherKey^^`, the same mechanism `**SkillName**` already uses for skill links.
    - **Entry points:** inline wherever `^^term^^` appears in any `desc`/`durNote`/status text, or via 📖 `#btnMechanicGlossary` in the hub-level `.hub-bar` (next to the changelog ↻ button, not scoped to any one tool) which opens the same modal on a browsable index (`renderMechanicIndex()`) of every mapped topic — `openMechanicPanel(null)` renders the index, `openMechanicPanel(key)` renders one topic.
    - **Color choice:** teal, checked against the full existing palette first (every other hue was already claimed: red=ATK, blue=DEF, green=AGI/HP, yellow=VIT, pink=INT, violet=CHA/`--status-kw`, cyan-blue=TAL, orange=LCK, slate=LV, gold=brand accent, oxblood=`--seal`). Shifted toward emerald rather than TAL's blue-leaning cyan so the two read as distinct at a glance.
