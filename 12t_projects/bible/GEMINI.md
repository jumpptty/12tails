...
* **To wire when each card gets its `/sd` pass** (found by a source survey, all confirmed white + purple in one cast): Whale Combo 1–3 (Wall Puncture, replace), ~~Wolf Combo 1–4~~ (done 2026-09-24 with `effectDamageDep`, see below), Cat Open Wound #443 (bonus `30×(disarm+bleed)`, no gate on the damage, only the flash/message has a 0.1 s throttle; wired to Flying Dagger, Delta Strike, Disarm, Bleed on 2026-09-26 via `CAT_OPENWOUND_PROC`; Combo and the stub Tree B skills wait for their own cards), Mole Smart Shell (bonus 30: Time Nuke, Stun Grenade, Mine, Stun Mine), the remaining Panda Shadow Fist users (Combo 1–5). Needing their own design: Bison Colossal Weapon (splash onto *other* targets) and Bat Merciless Drain (uncertain decompiled condition).
