...
A hub tool (`mountChaAgiOptimizer`, tile "ตัวคำนวณ CHA / AGI") that shows, per skill, the cheapest CHA + AGI for **no downtime** (cooldown ≤ duration, so the buff can be recast before it ends) No stat inputs and no explanatory text lines (user decision): each card is two numbers, CHA and AGI, each with the base value its stat scales (CHA box: base duration, AGI box: base cooldown).
...
* **No AGI-only threshold (removed 2026-09-25):** +1 CHA adds 1.5% of the *base* duration, i.e. `1/(CHA+66.7)` of the current one, while +1 AGI cuts the cooldown by `1/(AGI+129)`; AGI is the better point iff `AGI + 129 < CHA + 66.7`. The cut-off moves with CHA, so a fixed AGI number cannot express it (against a fixed 1.5% AGI never wins, its best is 0.78%). A relative "CHA − AGI gap" box was tried and dropped at the user's request.
