...
A hub tool (`mountChaAgiOptimizer`, tile "ตัวคำนวณ CHA / AGI") that shows, per skill, the cheapest CHA + AGI for **no downtime** (cooldown ≤ duration, so the buff can be recast before it ends) and one **AGI threshold**. No stat inputs (user decision: each skill only needs its best combination and the threshold).

* **Skills:** fixed list `CAO_SKILL_IDS` (user-chosen: Wolf Dark Edge, Wolf Lunar Eclipse, Rabbit Rapid Trance, Chameleon Immunity). Cooldown/duration/rank/icon are read from each skill's own `SKILLS` card, so card fixes flow through; add a skill only after checking its cast site uses a plain `agiAdjust(cd)` cooldown and a plain `chaAdjust(dur)` duration. Wolf Perseverance applies to a card whose `dep` is `WOLF_PERSEVERANCE_DEP`. Controls: Revised Art toggle, Perseverance 0/1/2, per-card rank buttons.
