...
A hub tool (`mountChaAgiOptimizer`, tile "ตัวคำนวณ CHA / AGI") that shows, per skill, the cheapest CHA + AGI for **no downtime** (cooldown ≤ duration, so the buff can be recast before it ends) No stat inputs and no explanatory text lines (user decision): each card is two numbers, the target CHA and AGI, each with the base value its stat scales (CHA box: base duration, AGI box: base cooldown), plus one shared rule line under the title.
...
* **Validator:** `[CAO ERROR]`: 36 cooldown/duration/Perseverance/Revised Art cases against an independent brute force, the ~61 rule line within 4.5 pp uptime of the best split at every budget and ending on the target, known totals (Dark Edge 522 / 354, Lunar Eclipse 632 / 439), and the real mount (4 cards, toggle re-render, Ctrl+Z).
