...
A hub tool (`mountChaAgiOptimizer`, tile "ตัวคำนวณ CHA / AGI") that shows, per skill, the cheapest CHA + AGI for **no downtime** (cooldown ≤ duration, so the buff can be recast before it ends) No stat inputs and no explanatory text lines (user decision): each card is two numbers in fill order, "ลง AGI ก่อน ถึง <agi>" then "แล้วลง CHA ถึง <cha>", each with the base value its stat scales (AGI box: base cooldown, CHA box: base duration).
...
* **Fill order = AGI first, up to the optimal AGI, then CHA (2026-09-25).** Measured in **seconds of downtime** (`max(0, cooldown − duration)`), +1 CHA always adds the same `0.015 × base duration × Perseverance` seconds (Dark Edge r4 P2: 0.18s), while +1 AGI removes `128 × CD × 0.88 / (AGI+128)²` seconds, which shrinks with AGI only. So AGI wins until `AGI = K − 128` (smooth), independent of CHA. With exact rounding (brute force of the least-downtime split at every point budget) the split is all-AGI first, the switch point becomes a tie range, and the only exact value is the optimal AGI itself (Dark Edge 154 vs smooth 146). Following "AGI to the optimal AGI, then CHA" lands exactly on no downtime and is never more than ~2s of downtime behind the best split on the way (measured max 2.04s; validator bound 2.05s).
* **Not a percentage threshold (removed 2026-09-25):** +1 CHA adds 1.5% of the *base* duration, i.e. `1/(CHA+66.7)` of the current one, while +1 AGI cuts the cooldown by `1/(AGI+129)`; AGI is the better point iff `AGI + 129 < CHA + 66.7`. The cut-off moves with CHA, so a fixed AGI number cannot express it (against a fixed 1.5% AGI never wins, its best is 0.78%). A relative "CHA − AGI gap" box was tried and dropped at the user's request.
...
* **Validator:** `[CAO ERROR]`: 36 cooldown/duration/Perseverance/Revised Art cases against an independent brute force, the fill order's worst extra downtime ≤ 2.05s and ending at no downtime, known totals (Dark Edge 522 / 354, Lunar Eclipse 632 / 439), and the real mount (4 cards, toggle re-render, Ctrl+Z).
