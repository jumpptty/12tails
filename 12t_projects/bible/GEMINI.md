...
* **To wire when each card gets its `/sd` pass** (found by a source survey, all confirmed white + purple in one cast): Whale Combo 1–3 (Wall Puncture, replace), Wolf Combo 1–4 (`darkEdge` status turns the whole hit purple; Combo 4 checks per swing), Cat Open Wound #443 (bonus `30×(disarm+bleed)` with an internal cooldown, 15 skills), Mole Smart Shell (bonus 30: Time Nuke, Stun Grenade, Mine, Stun Mine), the remaining Panda Shadow Fist users (Combo 1–5). Needing their own design: Bison Colossal Weapon (splash onto *other* targets) and Bat Merciless Drain (uncertain decompiled condition).
