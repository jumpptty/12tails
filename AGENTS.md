...
* **Per-Class `SKILLS` Ordering (added 2026-09-18, Monkey/Bat pilots):** Within a class's block in the `SKILLS` array, order entries by that class's own internal skill ID — the exact sequence `<Class>Skill.cs`'s `getSkillTree()` function emits its `result = "<class>_<name><rank>"` assignments in. This IS the game's own ID-ascending order; ignore the obfuscated bogus-arithmetic conditions guarding each assignment (per Section 3's Junk Predicates rule) and read only the literal sequence of `result = ...` lines. Collapse to one entry per skill family (drop the trailing rank-number suffix) in first-appearance order. Exclude the 4 universal Common skills (`statPlus`/`superStatPlus`/`revisedSkill`/`revisedMagic`/`revisedArt` — see the Shared/Universal Skills Exemption in Section 5) since those are authored once under the Common tile, not duplicated per class. Re-derive the sequence fresh per class from that class's own `getSkillTree()` — don't assume it mirrors another class's branch structure (Monkey's tree splits into parallel Fire/Earth columns; Bat's is a single linear sequence with a few interleaved pairs, e.g. `dissolute1, corruption1, dissolute2, corruption2`). Not yet enforced by `scripts/validate_skills.js` (most classes haven't been reordered yet, and the validator only reads `index.html` — it doesn't parse `DecompiledSource/*.cs` to derive the expected order itself).
...
**Mandatory Reference Review First (added 2026-09-18):** Before starting any decompiled source tracing or deep-dive research, the agent MUST read `12t_reference/12Tails-Mechanics-Reference.md` (and the corresponding `*-skill-reference.md` / `*-skill-damage-reference.md`) to check if the global mechanic, formula, stat adjuster, or damage pipeline has already been verified and documented. Never spend turns re-researching solved mechanics (such as `hitMod`, `defAdjust`, `chaAdjust`, derived stats, or class growth tables).

**Mandatory Engine Findings Recording Rule (added 2026-09-18):** Whenever research reveals a new global formula, game-wide engine mechanic, status classification behavior, or cross-class system (such as `hitMod`, SP generation rules, cleanse interactions, or derived stats) that is not yet documented in `12t_reference/12Tails-Mechanics-Reference.md`:
1. **Immediate Document Update:** The agent MUST append or update the verified findings into `12t_reference/12Tails-Mechanics-Reference.md` with complete source citations (`file:line`) and decompiled snippets.
2. **Class-Specific vs Global Separation:** If the finding is specific to one class (e.g. a multi-hit loop or class damage formula), record it in `12t_reference/<class>-skill-damage-reference.md`. If it affects the global engine or multiple classes, record it in `12Tails-Mechanics-Reference.md`.
3. **Never Leave Ground Truth Solely in Code:** Never leave newly discovered mechanics trapped only in `index.html` card objects or chat responses.

**Shared/Universal Skills Exemption (added 2026-09-16):** Five skills share identical numeric skill IDs and mechanics across all 12 classes (verified: same IDs/effects in `Monkey.cs`, `CharacterControl.cs`, `CharacterDataClass.cs`, cross-checked against `PenguinSkill.cs`; documented in [12Tails-Mechanics-Reference.md §3.5](12t_reference/12Tails-Mechanics-Reference.md#35-universal-shared-skills-characterdataclasscs-charactercontrolcs)) — Revised Art (`#424`), Revised Magic (`#414`), Revised Skill (`#404`), Stat Plus (`#141-144`), and Super Stat Plus (`#441`). These are authored **once**, under a dedicated `class: "Common"` tile in `12t_projects/bible/index.html` (`common_revisedArt`, `common_revisedMagic`, `common_revisedSkill`, `common_statPlus`, `common_superStatPlus`), not duplicated per class. **Skip these five when running the per-class research pipeline below** — reference the existing Common cards instead of re-deriving them. The separate 12-card Common support batch (Replenishment through Divine Channel) is now fully researched and authored; unlike the five universal skills, each card has its own source-class level formula and must retain its individual source proof and `supportSourceClass` / `supportFormula` wiring.
...
* Run a temporary scratch script (in `<appDataDir>/brain/<conversation-id>/scratch/`) or `view_file` to trace the full lifecycle across:
  0. **Cost/Mode/Req Table (added 2026-09-17):** Run `scripts/decode_skilldata.py <Class>Skill.cs` for every skill's MP/SP/reqLv before reading `getSkill()`'s obfuscated fallthrough chain by eye. `SkillClass.setMP`/`setSP` use first-setter-wins semantics inside a chain of shared `goto`-linked tail blocks — a manual read easily misattributes a later call in the chain as the real value (this produced a wrong Ja-cost guess of 55 MP before the decoder gave the correct, verified 15/30/45/60 per rank). The script's `SP` field sign doubles as the red/blue indicator (negative = red/spent, positive = blue/gate-only, `0` = no SP cost). Only fall back to manual reading of `getSkill()` if the script errors on that class file.
  ...
  3. **Status Effect Tracing (Name, Level, and Classification):**
     ...
     * **Status Classification (via `StatusData.cs`, see [12Tails-Mechanics-Reference.md §4.2](12t_reference/12Tails-Mechanics-Reference.md#42-status-classification--cleanse-system-statusdatacs)):** Must verify and report exact boolean return values:
     ...
     * **Debuff Duration Contesting Audit:** Any status or debuff whose duration is calculated via `Damage.getDebuff(...)` MUST declare both `durWrapped: true` and `durContested: true` in its card definition. For inverted debuffs calculated through `Damage.getDebuffInvert(...)`, declare `durContestedInverted: true`.
  ...
  7. **Summon / Companion Trigger (MANDATORY):** If the skill spawns an entity, deploys a turret, or commands a companion (e.g., `RPC_SpawnBarrelBot`, `RPC_CreatePet`, `RPC_KingKaiser`, `BarrelBot.cs`, `Phoenix.cs`, `Gadina.cs`):
     * **IMMEDIATELY activate and strictly adhere to [Section 7](#7-summon-mechanics-companion-movesets--summon-stat-cards), [Section 8](#8-compatible-skills-navigation-compatskills-conventions), and [12Tails-Mechanics-Reference.md §4.3](12t_reference/12Tails-Mechanics-Reference.md#43-summon--companion-entity-mechanics).**
...
6. **Proposed Header Tooltip (`desc`):**
   ...
   * **Omit Universal Always-On Side Effects (added 2026-09-17):** Don't restate a mechanic baked into a shared, engine-level system that fires identically across classes and skills, regardless of which one is on the card (see [12Tails-Mechanics-Reference.md §1.2](12t_reference/12Tails-Mechanics-Reference.md#12-derived-values-charactercontrolcs14641479)) — it adds no class-specific information and just clutters every desc it touches. Confirmed example: **+1 SP per landed basic-attack hit**, written into each class's own `nAttack` coroutine in near-identical form (`Monkey.cs:20055`, `Wolf.cs:15292`, `Bat_nAttack.cs:294`, etc.). Distinct from `CharacterControl.cs:2122`'s `this.sp++` inside the shared `ApplyDamage()` on damage taken. Applies only to effects this systemic and this uniformly duplicated — a passive-gated or class-specific proc still belongs in `desc` even if it superficially resembles this pattern.
...
7. **Proposed Active Card Definition (`index.html` Schema):** Show one complete schema block before requesting approval; do not substitute prose for any field.
   ...
   * **Complete Chip Audit:** Explicitly account for every applicable field—`cost` (including `spType: "red" | "blue"` whenever `sp` is specified), `cd`/`cdWrapped`, `castTime`/`castWrapped`, `duration`/`durWrapped` (or `durationInfinite`/`hideDurationChip`), `status`, `lckProc`, `ko`/`koDep`, `dmg`/`shield`/`dmgGroups`, hit count, and clamps. State `None` for each category that is not present.
     * **Always Omit `setReq`/`reqLv`/`reqBn` (standing rule, added 2026-09-17):** A `skillClass.setReq(reqLv, reqBn)` call found while tracing the cost table is a level/skill-point learn-prerequisite — no card field exists for it and none should be added. Omit it silently; it does not need to appear in the remainder findings list either.
     * **SP Cost Conventions (`spType: "red" | "blue"`, enforced by `scripts/validate_skills.js`; see [12Tails-Mechanics-Reference.md §3.1](12t_reference/12Tails-Mechanics-Reference.md#signed-sp-cost-semantics-csp)):**
       - **Red SP (`spType: "red"`)**: Consumed SP (`cSP < 0`). Requires and **consumes** SP on cast (`GameGui.cs:37782-37807`), halved by Revised Skill (#404). Rendered in-game as red text `new Color(1f, 0.2f, 0.2f)`. Standard for almost all physical skills.
       - **Blue SP (`spType: "blue"`)**: Threshold/Activation SP (`cSP > 0`). Checks that the player currently possesses at least that amount of SP (`GameGui.cs:37609`), but **does NOT consume SP** on cast. Rendered in-game as cyan-blue text `new Color(0.2f, 0.6f, 1f)`. (e.g. Whale's `rejuvenate` Ranks 3–4).
...
* **Generalized (added 2026-09-17): this isn't limited to dep objects.** Any verified finding on an *already-shipped* card, discovered incidentally while researching an unrelated skill, gets surfaced to the user immediately in the same turn — never silently deferred, never silently fixed without asking. Concrete precedent: while tracing Damage Cast's `FireRune()` call, Fire Rune's own already-shipped `desc` turned out to be missing that exact proc source from its own supported-sources list. Same rule as the dep-object case above, just without a dep object as the trigger — the trigger is "I now have proof this other card is wrong or incomplete," full stop.
