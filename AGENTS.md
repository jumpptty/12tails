...
1. **Bidirectional Policy (revised 2026-09-15, per direct user request — replaces the prior "main skill only" hub rule):** Every `compatSkills` edge must be reciprocated. If skill A lists skill B, skill B's own `compatSkills` must list A back. This applies uniformly to summon/mass-cast hubs (e.g. `bat_massCast` → its 15 spell targets; `mole_barrelBot`/`mole_kingKaiser`/`mole_autoGyroGun` → their child moves) and to small skillDep clusters (e.g. Ground Lock ↔ Aegis of Earth ↔ Second Stone) alike — there is no longer a distinct "hub" category with different rules.
   * This is a **reciprocal-edge rule, not a full sibling mesh**: a child/target only needs to link back to whichever parent(s) actually reference it (e.g. each of Barrel Bot's 8 moves lists just `["mole_barrelBot"]`; a move doesn't also need every sibling move listed). Only add a sibling-to-sibling edge when a real, separate mechanical tie exists between those two specific skills (e.g. `monkey_gadina_nAttack` lists `monkey_gadina`, `monkey_titanSword`, AND `monkey_earthSoul` because all three genuinely modify its own attack, not because they share a hub).
