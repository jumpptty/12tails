...
- Every card's displayed value must trace back to a formula/constant recorded in `12t_reference/penguin-skill-damage-reference.md` — no invented numbers.
...
- **Create:** `12t_projects/penguin-skill-sheet/12_Penguin_skill-sheet.html` — the shipped single-file sheet (styles + engine + data + generated markup). Placed alongside the existing `12t_projects/stat-gain-tables/11_Penguin_stat-gain-table.svg` per this repo's existing numbering convention for Penguin mechanics docs.
...
- **Reference only (no edits):** `12t_reference/penguin-skill-damage-reference.md` (all formula/mechanic data), `docs/superpowers/specs/2026-07-21-penguin-interactive-infographic-design.md` (design decisions), `C:\Users\Jump\.claude\skills\12tails-class-infographic\example-template.html` (visual conventions to port).
...
**Files:**
- Create: `12t_projects/penguin-skill-sheet/12_Penguin_skill-sheet.html`
...
Open `12t_projects/penguin-skill-sheet/12_Penguin_skill-sheet.html` directly in a browser (e.g. `start` the file on Windows, or drag into a browser tab). Open DevTools console.
...
**Files:**
- Modify: `12t_projects/penguin-skill-sheet/12_Penguin_skill-sheet.html` (append to the existing `<script>` block)
...
Append the final (verified) `tokenizeFormula`, `parseFormula`, `ADJUSTER_TAGS`, `renderSymbolic`, `renderNumeric`, `formatNumber` functions into `12t_projects/penguin-skill-sheet/12_Penguin_skill-sheet.html`'s `<script>` block, after the `SKILLS = {}` line from Task 2.
...
**Files:**
- Modify: `12t_projects/penguin-skill-sheet/12_Penguin_skill-sheet.html`
...
Open `12t_projects/penguin-skill-sheet/12_Penguin_skill-sheet.html` in a browser.
...
Source every value from `12t_reference/penguin-skill-damage-reference.md`'s "BASIC (1xx)" section — do not re-derive from source files, that extraction is already done and verified.
...
Reload `12t_projects/penguin-skill-sheet/12_Penguin_skill-sheet.html`.
...
Run: `grep -n "http://\|https://\|file:///" "12t_projects/penguin-skill-sheet/12_Penguin_skill-sheet.html"`
...
Use the Artifact tool with `file_path` pointing at `12t_projects/penguin-skill-sheet/12_Penguin_skill-sheet.html`, `favicon` set to a penguin/snowflake emoji (e.g. `❄️`), and a `description` summarizing it as an interactive Penguin skill sheet with rank toggles and a live stat calculator.
